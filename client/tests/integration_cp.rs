...
fn test_driver_copy_file() {
    ...
    let client: Fs<LocalDriver> = LocalDriver::connect("").unwrap().into();

    let content = "Hello, World!";
    let file_src = root_path.join("src.txt");
    let mut writer = client.write(&file_src, WriteMode::FailIfExist).unwrap();
    writer.write_all(content.as_bytes()).unwrap();
    drop(writer);

    let file_dst = root_path.join("dst.txt");

    client.copy_file(&file_src, &file_dst).unwrap();
    assert!(client.exists(&file_dst), "Файл должен существовать");
...
fn test_driver_copy_dir() {
    ...
    client.copy(&src_dir, &dst_dir).unwrap();
    assert!(client.exists(&dst_dir), "Директория должна существовать");
